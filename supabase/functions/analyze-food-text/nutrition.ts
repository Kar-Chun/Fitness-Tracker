...
export function resolveUSDAItem(item: ParsedFoodItem, match: USDAFoodMatch): ResolvedFoodItem {
  ...
  return {
    ...
    calorieRange: {
      low: Math.max(1, Math.round(nutrients.calories * (1 - factor))),
      high: Math.max(nutrients.calories, Math.round(nutrients.calories * (1 + factor))),
    },
    source: "usda",
...
export function resolveAIItem(item: ParsedFoodItem): ResolvedFoodItem {
  ...
  return {
    ...
    estimatedGrams: item.estimatedGrams,
    calories,
    proteinG: Math.max(0, Math.round(item.fallbackProteinG * 10) / 10),
    calorieRange: {
      low: Math.max(1, Math.min(calories, Math.round(item.fallbackCalorieLow))),
      high: Math.max(calories, Math.round(item.fallbackCalorieHigh)),
    },
    source: "ai_estimate",
...
export function buildEstimate(meal: ParsedMeal, items: ResolvedFoodItem[]): FoodEstimate {
  ...
  return {
    ...
    confidence: determineConfidence(items),
    source,
    sourceSummary: sourceSummary(items, source),
    items,
  }
}
...
export function buildPersonalEstimate(match: PersonalMatch): FoodEstimate {
  ...
  const item: ResolvedFoodItem = {
    name: match.name,
    portionDescription: "Based on your previous value",
    estimatedGrams: 0,
    calories,
    proteinG: Math.round(match.proteinG * 10) / 10,
    calorieRange: { low: Math.max(1, calories - margin), high: calories + margin },
    source: match.source,
    confidence: "high",
    caloriesPer100g: null,
    proteinPer100g: null,
