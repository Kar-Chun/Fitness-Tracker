import { normalizeFoodName } from "../_shared/food-normalization.ts"
import type {
  FavouriteFood,
  HistoryFood,
  PersonalMatch,
...
function normalizeMealLookup(value: string) {
  ...
  MEAL_MODIFIERS.forEach((modifier) => {
    normalized = normalized.replace(new RegExp(`\\b${modifier}\\b`, "g"), " ")
  })
  return normalized.replace(/\s+/g, " ").trim()
}

export function findPersonalMatch(
  query: string,
  history: HistoryFood[],
  ...
  const historyMatch = [...history]
    ...
    .find((entry) => normalizeMealLookup(entry.name) === normalizedQuery)
  if (historyMatch) {
    return {
      name: historyMatch.name,
      calories: historyMatch.calories,
      proteinG: historyMatch.protein_g ?? 0,
      source: "history",
      note: `Based on your previous ${historyMatch.name} entry.`,
    }
  }
  ...
  if (favourite) {
    return {
      ...
      proteinG: favourite.protein_g ?? 0,
      source: "favourite",
      note: `Based on your ${favourite.name} favourite.`,
    }
  }

  const savedMeal = savedMeals.find((meal) => normalizeMealLookup(meal.name) === normalizedQuery)
  if (savedMeal?.saved_meal_items.length) {
    return {
      name: savedMeal.name,
      ...
      source: "saved_meal",
      note: `Based on your saved meal ${savedMeal.name}.`,
    }
  }
...
function tokens(value: string) {
  return new Set(normalizeFoodName(value).split(/[^a-z0-9]+/).filter((token) => token.length > 1))
}
...
export function scoreUSDACandidate(query: string, candidate: USDAFoodCandidate) {
  ...
  IMPORTANT_TERMS.forEach((term) => {
    if (normalizedQuery.includes(term) && !normalizedDescription.includes(term)) score -= 0.22
  })
  UNRELATED_PRODUCT_TERMS.forEach((term) => {
    if (!normalizedQuery.includes(term) && normalizedDescription.includes(term)) score -= 0.16
  })
  return score
}
...
export function rankUSDACandidates(query: string, candidates: USDAFoodCandidate[]): USDAFoodMatch | null {
  const ranked = candidates
    .map((candidate) => {
      const nutrients = extractNutrientsPer100g(candidate.foodNutrients ?? [])
      return { candidate, score: scoreUSDACandidate(query, candidate), nutrients }
    })
    .filter((result) => result.nutrients.caloriesPer100g > 0)
  ...
  return {
    ...
    score: best.score,
    caloriesPer100g: best.nutrients.caloriesPer100g,
    proteinPer100g: best.nutrients.proteinPer100g,
  }
}
